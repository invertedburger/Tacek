...
def generate(sources, timestamp, today=None):
    ...
    for i, src in enumerate(sources):
        ...
        no_menu = src.get('no_menu', False)

        if no_menu:
            cards_html += f"""
      <div class="anim-card bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-red-200 dark:border-red-900/60 flex flex-col opacity-60" style="animation-delay:{i * 80}ms">
...
{map_section}
  <footer class="text-center text-gray-300 dark:text-gray-600 text-xs py-8 space-y-1">
    <div><span data-i18n="card.updated">{i18n.cs('card.updated')}</span> {timestamp}
    &middot; <span data-i18n="footer.next">{i18n.cs('footer.next')}</span> <span id="next-update">&mdash;</span>
    &middot; <span data-i18n="footer.index">{i18n.cs('footer.index')}</span></div>
